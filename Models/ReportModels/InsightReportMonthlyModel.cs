...
public class InsightReportMonthlyModel : InsightReportBaseModel
{
    ...
    )
    {
        ...
        var beginDate = ConvertBeginDate(beginYearMonth);
        var endDate = ConvertEndDate(endYearMonth);

        Items = new List<InsightSummaryItem>();

        for (DateOnly date = beginDate; date <= endDate; date = date.AddMonths(1))
        {
            var intDate = date.Year * 100 + date.Month;
